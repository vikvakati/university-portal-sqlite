...
#Test code done by Matt Taylor
class Test(unittest.TestCase):
    def setUp(self): #used for multiple tests in one run
        pass
    def test_searchcourse(self): #Searches for the course in the list, set up with 5 values as of now for testing purposes
        courses = ["Chemistry", 1002, "Patrick", "TR", "4"]
        for i in range(5):
            course = courses[i]
            actual = searchCourse(course)
            self.assertEqual(actual, 1)
    def test_printcourse(self): #Prints out all of the courses in the DB and returns a value to confirm functionality
        actual = printCourse()
        self.assertEqual(actual, 1)
...
#Function added by Matt, just avoids user input and takes direct input for testing purposes
def searchCourse(course):
    x = course
    cursor.execute("""SELECT * 
    FROM COURSE 
    WHERE CRN='%s' OR TITLE='%s' OR DEPARTMENT='%s' OR INSTRUCTOR_FIRST='%s' OR 
    INSTRUCTOR_LAST='%s' OR TIME='%s' OR DAYS='%s' OR SEMESTER='%s' OR YEAR='%s' OR 
    CREDITS='%s' """% (x, x, x, x, x, x, x, x, x, x))
    courseInput_result = cursor.fetchall()
    for i in courseInput_result:
         print(i)
    return 1

#Function added by Matt to print all courses and return value. For testing purposes
def printCourse():
    cursor.execute("""SELECT * 
    FROM COURSE""")
    courseInput_result = cursor.fetchall()
...
z = 0
while(z == 0):
    unittest.main() #Runs the tests for the program and bypasses user input. Breaks when tests finish
